...
$('.project-nav #prev').click(function (e) {
	...
	$('.projects').animate({left: fullLeft - 1024 + 'px'});

	if (fullLeft == -2048) {
		$('.projects').animate({left: '0px'});
	};
});
...
$(window).scroll(function(){
	...
	if(wScroll > $('#events').offset().top - $(window).height()){
	    ...
	    $('.event-1').css({'transform': 'translate('+ offset +'px, '+ Math.abs(offset * 0.2) +'px)'});

	    $('.event-3').css({'transform': 'translate('+ Math.abs(offset) +'px, '+ Math.abs(offset * 0.2) +'px)'});
